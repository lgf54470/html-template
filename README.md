...
- **新增**:必填简体中文名 + 英文名,可选繁体中文名与备注;英文名经 slug 化生成唯一 id(`ws-<英文名称>`,重名自动追加 `-2/-3…`),并可选择预设图标与主题面板的 18 种强调色(zinc + 17 种 chart 色)。
- **编辑 / 删除**:鼠标悬停工作空间条目出现编辑、删除按钮;可改简体/繁体名、备注、图标与颜色(英文名作为标识,创建后不可修改),删除会同时清理该工作空间的全部数据,且至少保留一个工作空间。
...
└── js/
    ...
    │   ├── ui.js             # 共享 UI 组件(button/badge/card/radio-group/avatar/placeholder/404)
    │   ├── avatar.js         # 用户头像交互:上传图片裁剪压缩(200KB 限制)+ 选择器事件
    │   ├── shell.js          # App Shell 渲染(侧边栏/顶栏/设置面板)
    │   ├── app.js            # 应用内核:模块注册表 + Hash 路由 + 事件委托 + 设置双向同步
    │   ├── workspace.js      # 工作空间交互:切换 + 新增/编辑/删除弹窗(双语名称/备注/图标/强调色)
